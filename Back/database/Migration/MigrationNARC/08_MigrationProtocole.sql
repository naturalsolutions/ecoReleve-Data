...
select S.*
		   ...
		   AND EXISTS (SELECT * FROM [NARC_eReleveData].dbo.TStations S2 where S.TSta_PK_ID <> s2.TSta_PK_ID and isnull(S2.LAT,-1) =isnull(S.LAT,-1) and isnull(S2.LON,-1) =isnull(S.LON,-1) and S.[DATE] = S2.[DATE] AND S2.FieldActivity_ID != 27)
...
SET IDENTITY_INSERT  [Station] ON
	...
	INSERT INTO [dbo].[Station]
		   ...
		   select DISTINCT S.TSta_PK_ID, isnull([DATE],creation_date)
		   ,S.Name,S.LAT,S.LON,S.ELE,S.precision,FA.ID,S.creator,S.Creation_date,st.id,R.ID,MS.ID
		   ,S.Place,'eReleve_'+CONVERT(VARCHAR,S.TSta_PK_ID),s.Comments
		   from [NARC_eReleveData].dbo.TStations S 
		   LEFT JOIN fieldActivity FA on FA.Name = S.FieldActivity_Name 
		   JOIN StationType st ON  st.name = 'standard'
		   LEFT JOIN Region R on r.Region = S.Region
		   LEFT JOIN MonitoredSite MS ON MS.original_id = S.TSta_FK_TGeo_ID
		   WHERe NOT EXISTS (select * from Station S2 where S2.LAT = S.LAT AND S2.LON = S.LON AND s2.LAT = S.LAT and S.[DATE] = S2.StationDate)
		   AND (S.FieldActivity_ID != 27 or S.FieldActivity_ID IS NULL)
...
,s.ID
  FROM Station s 
  JOIN [NARC_eReleveData].[dbo].[TStations] ts 
		on s.ID = ts.TSta_PK_ID
...
FROM [NARC_eReleveData].dbo.TStations S 
JOIN Station s2 ON S.TSta_PK_ID = s2.ID
