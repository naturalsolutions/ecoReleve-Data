...
from ecoreleve_server.core import HasDynamicProperties, GenericType
...
class Observation(HasDynamicProperties, MAIN_DB):
    ...
    @declared_attr
    def fk_table_type_name(self):
        return 'FK_ProtocoleType'

    @classmethod
    def getTypeClass(self):
        if not hasattr(self, 'TypeClass'):
            class Type(GenericType, MAIN_DB):
                __name__ = 'ProtocoleType'
                __tablename__ = 'ProtocoleType'
                obsolete = Column(Boolean)
                parent = self
            self.TypeClass = type('ProtocoleType', (Type, ), {})
        return self.TypeClass

    @declared_attr
    def _type_id(self):
        Type = self.getTypeClass()
    ...
    def linkedFieldDate(self):
        try:
            Station = MAIN_DB.metadata.tables['Station']
            if not self.Station:
                linkedDate = self.session.execute(
                    select([Station.c['StationDate']])
                        .where(Station.c['ID'] == self.FK_Station)).scalar()
            else:
    ...
    def SubObservation_childrens(self):
        dictToUpdate = {}
        if (self.SubObservation_children is not None or
           self.SubObservation_children != []):
