...
def getIndiv(request):
    ...
    if 'FormName' in request.params:
        ...
        for row in dataResult:
            geoJson.append({'type': 'Feature', 'properties': {'type': row['type_'], 'sensor': row['UnicIdentifier'], 'date': row[
                           'Date'], 'ID': row['ID']}, 'geometry': {'type': 'Point', 'coordinates': [row['LAT'], row['LON']]}})
...
def checkExisting(indiv):
    ...
    searchInfo = {'criteria': [{'Column': key, 'Operator': 'is', 'Value': val}
                               for key, val in indivData.items()], 'order_by': ['ID:asc']}
...
def searchIndiv(request, searchInfo=None, noCount=False):
    ...
    if 'typeObj' in request.params:
        ...
        searchInfo['criteria'].append(
            {'Column': 'FK_IndividualType', 'Operator': '=', 'Value': request.params['typeObj']})
    else:
        searchInfo['criteria'].append(
            {'Column': 'FK_IndividualType', 'Operator': '=', 'Value': 1})
        typeObj = 1
