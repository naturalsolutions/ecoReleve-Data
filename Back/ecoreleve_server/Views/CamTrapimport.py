...
def uploadFileCamTrapResumable(request):
    ...
    if(int(request.POST['resumableChunkNumber']) == 1 and int(request.POST['resumableCurrentChunkSize']) == int(request.POST['resumableTotalSize']) and str(extType[len(extType) - 1]) != ".zip"):
        if not os.path.isfile(pathPrefix + '\\' + pathPost + '\\' + str(request.POST['resumableFilename'])):
            # write in the file
            with open(uri + '\\' + str(request.POST['resumableFilename']), 'wb') as output_file:
                shutil.copyfileobj(inputFile, output_file)
            output_file.close()
        datePhoto = dateFromExif(
            uri + '\\' + str(request.POST['resumableFilename']))
        if(checkDate(datePhoto, str(request.POST['startDate']), str(request.POST['endDate']))):
            AddPhotoOnSQL(fk_sensor, str(uri), str(
                request.POST['resumableFilename']), str(extType[len(extType) - 1]), datePhoto)
            resizePhoto(str(uri) + "\\" +
                        str(request.POST['resumableFilename']))
            messageDate = "ok <BR>"
        else:
        ...
        if not os.path.isfile(pathPrefix + '\\' + pathPost + '\\' + str(request.POST['resumableIdentifier'])):
            ...
            position = int(request.POST['resumableChunkNumber'])
            # write in the file
            with open(uri + '\\' + str(request.POST['resumableIdentifier']) + "_" + str(position), 'wb') as output_file:
                shutil.copyfileobj(inputFile, output_file)
            output_file.close()
                ...
                if not os.path.isfile(pathPrefix + '\\' + pathPost + '\\' + str(name)):
                    ...
                    with open(pathPrefix + '\\' + pathPost + '\\' + str(name), 'wb') as destination:
                        ...
                        for i in range(1, int(request.POST['taille']) + 1):
                            ...
                            if os.path.isfile(pathPrefix + '\\' + pathPost + '\\' + str(request.POST['name']) + '_' + str(i)):
                                shutil.copyfileobj(open(pathPrefix + '\\' + pathPost + '\\' + str(
                                    request.POST['name']) + '_' + str(i), 'rb'), destination)  # on le concat dans desitnation
                                socketConcat.write("%s\n" % (i))
                                socketConcat.flush()
                                if (i == 30):
        ...
        if(message == ""):
            if(request.POST['type'] == "application/x-zip-compressed"):
                ...
                messageUnzip, nbInZip, nbInserted = unzip(pathPrefix + '\\' + pathPost + '\\' + str(
                    name), pathPrefix + '\\' + pathPost + '\\', fk_sensor, str(request.POST['startDate']), str(request.POST['endDate']))
                # print(messageUnzip)
                if(messageUnzip != ""):
                    request.response.status_code = 510
